...
##################################################################
#
# Processing Script for Residential Mobility by Race and Ethnicity
...
for(file in list.files(raw_location, pattern = "ACS_1.*")) {
    ...
    raw[,c("Variable", "Mobility", "Race/Ethnicity") := do.call(Map, c(f = c, str_split(Variable, fixed(";"))))]
    ...
    setkey(estimates, FIPS, Mobility, `Race/Ethnicity`)
    ...
    setkey(moes, FIPS, Mobility, `Race/Ethnicity`)
    ...
    setkey(totals, FIPS, `Race/Ethnicity`)
    setkey(numbers, FIPS, `Race/Ethnicity`)
...
dataset$`Race/Ethnicity` <- str_to_title(dataset$`Race/Ethnicity`)
...
#Sort columns
dataset <- dataset %>% 
  arrange(Town, Year, `Race/Ethnicity`, desc(Variable))

# write to file - new NA value will be "-6666" for values that are incalculable or inestimable
write.table (
  dataset,
  file.path(getwd(), "data", "residential_mobility_by_race_2015.csv"),
  sep = ",",
  row.names = F,
  na = "-6666"
)